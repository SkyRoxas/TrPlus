(function() {
  ...
  const minifySASS = function() {
    return gulp.src(config.src.sass).pipe(sass({outputStyle: 'expanded'}).on('error', sass.logError)).pipe(gulp.dest(config.dest.sass)).pipe(browserSync.stream())
  }

  const minifyHTML = function() {
    return gulp.src(config.src.html).pipe(pug({outputStyle: 'expanded'}).on('error', function(err){console.log(err)})).pipe(gulp.dest(config.dest.html)).pipe(browserSync.stream())
  }
